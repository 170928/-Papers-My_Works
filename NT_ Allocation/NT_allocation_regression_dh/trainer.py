...
class Trainer:
    def __init__(self,model,args,lr_straegy = None):
        ...
        self.num_comb = len(np.array(list(itertools.permutations(np.arange(0,args.num_nt,1),args.num_user))))
        ...
        self.cost = tf.reduce_mean(tf.losses.mean_squared_error(self.Y,self.model.logit))
        ...
        self.correct_prediction = tf.equal(self.Y, tf.rint(self.model.logit_softmax))
        ...
        #===============================================
            ...
            #==================================================
                ...
                for j in range(0, self.num_samples):
                    ...
                    labels[j] = np.argmax(data_gen.label)
            ...
            #==================================================
                ...
                rLogit, rCost, _,acc = sess.run([self.model.logit, self.cost, self.optimizer, self.accuracy],
                                            feed_dict={self.model.X: inputs, self.Y: labels.reshape(-1,1), self.model.trainphase: True,
                                                       self.lr_tensor: lr})

                optvals = np.array([data_gens[i].sumrateSet[labels[i]] for i in range(self.num_samples)])
                ...
                dnnvals = np.array([data_gens[i].sumrateSet[np.clip((int(np.rint(rLogit[i]))),0,self.num_comb-1)] for i in range(self.num_samples)])

                print(itr, "Cost :", rCost, "\tAccuracy :", round(acc,6), "\tcur_lr:",lr)
                print(itr, "optindex[0]  :",labels[0] , "\tdnnindex[0] :",int(np.rint(rLogit[0])),"("+str(np.clip(int(np.rint(rLogit[0])),0,self.num_comb-1))+")")
